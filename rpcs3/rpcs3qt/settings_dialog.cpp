	...
	, m_emu_settings(std::move(emu_settings))
{
	...
	// Keep this order in sync with render_creator::renderers
	...
	const auto scaled_resolution = [resolution_scale_def](int percentage)
	{
		...
		{
			return tr("%1% (%2x%3) (Default)", "Resolution scale").arg(percentage).arg(1280 * percentage / 100).arg(720 * percentage / 100);
		}
	...
	const auto apply_renderer_specific_options = [r_creator, this](const QString& text)
	{
		...
		// MSL is generated by MoltenVK (Vulkan on macOS) and by the native Metal renderer
		const bool is_metal = (text == r_creator->Metal.name);
		ui->disableMslFastMath->setEnabled(is_vulkan || is_metal);
	};
...
#ifndef HAVE_VULKAN
	// This build has no Vulkan backend (e.g. the macOS Metal-only build).
	// Hide the Vulkan-only options instead of showing them permanently disabled.
	ui->asyncTextureStreaming->setVisible(false);
	ui->gb_vulkansched->setVisible(false);
	ui->disableVulkanMemAllocator->setVisible(false);
	ui->useReBAR->setVisible(false);
	ui->gb_exclusiveFullscreen->setVisible(false);
#endif
...
#ifdef __APPLE__
		// RPCS3 Metal fork: the upstream auto-updater is compiled out on macOS (it would install upstream Vulkan builds over this app)
		ui->gb_updates->setVisible(false);
#endif
