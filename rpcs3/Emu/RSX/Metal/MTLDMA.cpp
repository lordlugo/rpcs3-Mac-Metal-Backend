...
namespace mtl
{
	// 64 KiB blocks: a multiple of the 16 KiB Apple silicon host page, as required by newBufferWithBytesNoCopy.
	...
	{
		// Unified memory: a shared-storage MTLBuffer can alias the guest memory mapping directly.
		return true;
	}
	...
	void* dma_block::map_range(const utils::address_range32& range)
	{
		...
		{
			memory_mapping = static_cast<u8*>(allocated_memory->map(0));
	...
	void dma_block::unmap()
	{
		...
		{
			// Shared storage buffers are persistently mapped; just drop the cached pointer.
	...
	void dma_block::allocate(const render_device& dev, usz size)
	{
		...
		allocated_memory = std::make_unique<mtl::buffer>(dev, size, memory_location::host_visible, "DMA block");

		// Initialize memory contents. This isn't something that happens often.
		// Pre-loading the contents helps to avoid leakage when mixed types of allocations are in use
		// TODO: Fix memory lost when old object goes out of use with in-flight data.
		auto dst = static_cast<u8*>(allocated_memory->map(0));
	...
	void dma_block::free()
	{
		if (allocated_memory)
		{
			...
			// Move allocation to gc (in-flight command buffers may still reference it)
	...
	{
		auto source = head();
		return static_cast<u32>(source->base_address + source->allocated_memory->size() - 1);
	}

	u32 dma_block::size() const
	{
		return (allocated_memory) ? static_cast<u32>(allocated_memory->size()) : 0;
	}
	...
	void dma_block_EXT::allocate(const render_device& dev, usz size)
	{
		...
		// Zero-copy: the buffer aliases the guest memory mapping (base_address and size are 64K aligned, the super pointer
		// base is host-page aligned). Nothing is freed by Metal on destruction.
		allocated_memory = std::make_unique<mtl::buffer>(dev,
			vm::get_super_ptr<void>(base_address),
			size,
			"DMA passthrough block");
	...
	static bool test_host_pointer(u32 base_address, usz length)
	{
		// newBufferWithBytesNoCopy requires the whole range to be backed by a valid VM mapping.
		// RSX local memory is always fully committed; main memory can have holes that are paged in on fault.
		return rsx::get_location(base_address) == CELL_GCM_LOCATION_LOCAL ||
			vm::check_addr(base_address, 0, static_cast<u32>(length));
	}
	...
	void create_dma_block(std::unique_ptr<dma_block>& block, u32 base_address, usz expected_length)
	{
		...
		{
			allow_host_buffers = test_host_pointer(base_address, expected_length);
	...
	void unmap_dma(u32 local_address, u32 length)
	{
		...
		for (u32 block = start; block < end;)
		{
			if (auto found = g_dma_pool.find(block); found != g_dma_pool.end())
			{
				...
				{
					// Passthrough block. Must unmap from GPU (the buffer is released through the GC)
