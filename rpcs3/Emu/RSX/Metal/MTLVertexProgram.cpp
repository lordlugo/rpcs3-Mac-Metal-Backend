...
std::string MTLVertexDecompilerThread::getFloatTypeName(usz elementCount)
{
	return glsl::getFloatTypeNameImpl(elementCount);
}

std::string MTLVertexDecompilerThread::getIntTypeName(usz /*elementCount*/)
{
	return "ivec4";
}
...
void MTLVertexDecompilerThread::prepareBindingTable()
{
	u32 location = 0;
	mtl_prog->binding_table.vertex_buffers_location = location;
	...
	{
		mtl_prog->binding_table.cr_pred_buffer_location = location++;
	}

	std::memset(mtl_prog->binding_table.vtex_location, 0xff, sizeof(mtl_prog->binding_table.vtex_location));
	...
	for (const ParamType& PT : m_parr.params[PF_PARAM_UNIFORM])
	{
		...
		for (const ParamItem& PI : PT.items)
		{
			if (is_texture_type)
			{
				...
				mtl_prog->binding_table.vtex_location[id] = location++;
				continue;
			}
			...
			if (PI.name.starts_with("vc["))
			{
				...
				mtl_prog->binding_table.instanced_lut_buffer_location = location++;
				mtl_prog->binding_table.instanced_cbuf_location = location++;
				continue;
			}
		}
	}
...
void MTLVertexDecompilerThread::insertHeader(std::stringstream& OS)
{
	prepareBindingTable();
	...
	OS <<
		"layout(std430, set=0, binding=" << mtl_prog->binding_table.context_buffer_location << ") uniform VertexContextBuffer\n"
		"{\n"
		"	vertex_context_t vertex_contexts[];\n"
		"};\n\n";

	mtl::glsl::program_input context_input
	{
		.domain = glsl::glsl_vertex_program,
		.type = mtl::glsl::input_type_uniform_buffer,
		.set = mtl::glsl::binding_set_index_vertex,
		.location = mtl_prog->binding_table.context_buffer_location,
	...
	{
		OS <<
			"layout(std430, set=0, binding=" << mtl_prog->binding_table.cr_pred_buffer_location << ") readonly restrict buffer EXT_Conditional_Rendering\n"
			"{\n"
			"	uint cr_predicate_value;\n"
			"};\n\n";

		mtl::glsl::program_input predicate_input
		{
			.domain = glsl::glsl_vertex_program,
			.type = mtl::glsl::input_type_storage_buffer,
			.set = mtl::glsl::binding_set_index_vertex,
			.location = mtl_prog->binding_table.cr_pred_buffer_location,
	...
	mtl::glsl::program_input push_constants
	{
		...
		.push_constant = mtl::glsl::push_constant_ref{ .offset = 0, .size = 4 },
		.set = mtl::glsl::binding_set_index_vertex,
		.location = umax,
...
void MTLVertexDecompilerThread::insertInputs(std::stringstream& OS, const std::vector<ParamType>& /*inputs*/)
{
	...
	u32 location = mtl_prog->binding_table.vertex_buffers_location;
	for (const auto& stream : input_streams)
	{
		OS << "layout(set=0, binding=" << location << ") uniform usamplerBuffer " << stream << ";\n";

		this->inputs.push_back(mtl::glsl::program_input
		{
			.domain = glsl::glsl_vertex_program,
			.type = mtl::glsl::input_type_texel_buffer,
			.set = mtl::glsl::binding_set_index_vertex,
			.location = location++,
			.name = stream
		});
...
void MTLVertexDecompilerThread::insertConstants(std::stringstream& OS, const std::vector<ParamType>& constants)
{
	...
	for (const ParamType &PT : constants)
	{
		for (const ParamItem &PI : PT.items)
		{
			if (PI.name.starts_with("vc["))
			{
				if (!(m_prog.ctrl & RSX_SHADER_CONTROL_INSTANCED_CONSTANTS))
				{
					OS << "layout(std430, set=0, binding=" << mtl_prog->binding_table.cbuf_location << ") uniform VertexConstantsBuffer\n";
					OS << "{\n";
					OS << "	vec4 vc[];\n";
					OS << "};\n\n";
				...
				else
				{
					...
					OS << "layout(std430, set=0, binding=" << mtl_prog->binding_table.instanced_lut_buffer_location << ") readonly restrict buffer InstancingData\n";
					OS << "{\n";
					OS << "	int constants_addressing_lookup[];\n";
					OS << "};\n\n";

					in.location = mtl_prog->binding_table.instanced_lut_buffer_location;
					in.name = "InstancingData";
					in.type = mtl::glsl::input_type_storage_buffer;
					inputs.push_back(in);

					// 2. Bind actual constants buffer
					OS << "layout(std430, set=0, binding=" << mtl_prog->binding_table.instanced_cbuf_location << ") readonly restrict buffer VertexConstantsBuffer\n";
					OS << "{\n";
					...
					in.location = mtl_prog->binding_table.instanced_cbuf_location;
					in.name = "VertexConstantsBuffer";
					in.type = mtl::glsl::input_type_storage_buffer;
					inputs.push_back(in);
					continue;
			...
			{
				const int id = mtl::get_texture_index(PI.name);
				in.location = mtl_prog->binding_table.vtex_location[id];
				in.name = PI.name;
				in.type = mtl::glsl::input_type_texture;
				...
				{
					// Metal: 1D textures are 2D textures of height 1 (see getFunction)
...
void MTLVertexDecompilerThread::insertOutputs(std::stringstream& OS, const std::vector<ParamType>& /*outputs*/)
{
	for (auto &i : reg_table)
	{
		if (i.need_declare)
		{
			...
			OS << "layout(location=" << mtl::get_varying_register_location(i.name) << ") out "
...
void MTLVertexDecompilerThread::insertMainStart(std::stringstream& OS)
{
	...
	properties2.emulate_depth_clip_only = false;        // The precise path needs fp64, which MSL does not have
	properties2.low_precision_tests = false;
	properties2.require_explicit_invariance = false;
...
void MTLVertexDecompilerThread::insertMainEnd(std::stringstream& OS)
{
	OS << "}\n\n";
...
{
	m_device_props.emulate_conditional_rendering = mtl::emulate_conditional_rendering();
	m_shader = Decompile();
...
void MTLVertexProgram::Decompile(const RSXVertexProgram& prog)
{
	...
	std::string source;
	MTLVertexDecompilerThread decompiler(prog, source, parr, *this);
	decompiler.Task();
...
void MTLVertexProgram::Compile()
{
	if (g_cfg.video.log_programs)
		fs::write_file(fs::get_cache_dir() + "shaderlog/VertexProgram" + std::to_string(id) + ".glsl", fs::rewrite, shader.get_source());

	// Translation to MSL and the MTLLibrary build are deferred to the pipe compiler (first pipeline using this shader)
	handle = &shader;
}

void MTLVertexProgram::Delete()
{
	shader.destroy();
	handle = nullptr;
}

void MTLVertexProgram::SetInputs(std::vector<mtl::glsl::program_input>& inputs)
{
	for (auto &it : inputs)
	{
