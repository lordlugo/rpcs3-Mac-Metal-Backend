...
// Compute kernels (port of VK/VKCompute.h).
//
// The GLSL sources are the same as the VK backend's (Vulkan semantics); they are translated through
// glslang -> SPIR-V -> SPIRV-Cross MSL by glsl::create_compute_program(). Push constant blocks stay GLSL
// push_constant blocks (the program uploads them to the scratch heap), SSBOs are bound by GPU address.
//
// Differences to VK (Metal has no robust buffer access, so every kernel must bounds-check itself):
//  - cs_shuffle_base always carries a push-constant block with the byte length of the job and checks it.
//  - cs_fconvert_task passes the number of OUTPUT work items to the kernel (VK relied on robustness).
//  - cs_aggregator receives the word count as a push constant (no runtime-array length() needed in MSL).
...
namespace mtl
{
	...
	{
		std::string m_src;
		std::unique_ptr<glsl::program> m_program;
		...
		// Threads per threadgroup used at dispatch. Must equal the GLSL local_size of the kernel. 0 = use (optimal_group_size, 1, 1).
		u32 workgroup_size_x = 0;
		u32 workgroup_size_y = 1;
		u32 workgroup_size_z = 1;
		...
		virtual void bind_resources(mtl::command_list& /*cmd*/) {}

		void load_program(mtl::command_list& cmd);

		// invocations_* are threadgroup counts (vkCmdDispatch semantics)
		void run(mtl::command_list& cmd, u32 invocations_x, u32 invocations_y, u32 invocations_z);
		void run(mtl::command_list& cmd, u32 num_invocations);

		// Push constant upload helper (compute set)
		void push_constants(u32 offset, u32 size, const void* data);
	};

	struct cs_shuffle_base : compute_task
	{
		const mtl::buffer* m_data = nullptr;
		u32 m_data_offset = 0;
		u32 m_data_length = 0;
		u32 kernel_size = 1;
		...
		void set_parameters(mtl::command_list& cmd);

		// Plain byte shuffles: sets the bounds parameter and dispatches
		void run(mtl::command_list& cmd, const mtl::buffer* data, u32 data_length, u32 data_offset = 0);

	protected:
		// Dispatch with the parameters already set in m_params (used by the derived tasks)
		void run_impl(mtl::command_list& cmd, const mtl::buffer* data, u32 data_length, u32 data_offset);
	};
	...
	struct cs_interleave_task : cs_shuffle_base
	{
		...
		void run(mtl::command_list& cmd, const mtl::buffer* data, u32 data_offset, u32 data_length, u32 zeta_offset, u32 stencil_offset);
	};
	...
	struct cs_fconvert_task : cs_shuffle_base
	{
		...
		void run(mtl::command_list& cmd, const mtl::buffer* data, u32 src_offset, u32 src_length, u32 dst_offset)
		{
			u32 data_offset;
			...
			// Each work item consumes one 32-bit word of the narrower type. The kernel computes block_length = params[0].x / 4,
			// so for f32->f16 contraction (2 source words per item) pass half the source length. Without robust buffer access
			// the VK value would write past the end of the output range.
			const u32 bounds_length = (sizeof(From) > sizeof(To)) ? (src_length / 2) : src_length;

			m_params = { bounds_length, src_offset - data_offset, dst_offset - data_offset, 0 };
			cs_shuffle_base::run_impl(cmd, data, src_length, data_offset);
		}
	};
	...
	{
		virtual void run(mtl::command_list& cmd, const mtl::buffer* dst, u32 out_offset, const mtl::buffer* src, u32 in_offset, u32 data_length, u32 width, u32 height, u32 depth, u32 mipmaps) = 0;
	};
	...
	struct cs_deswizzle_3d : cs_deswizzle_base
	{
		...
		const mtl::buffer* src_buffer = nullptr;
		const mtl::buffer* dst_buffer = nullptr;
		u32 in_offset = 0;
		u32 out_offset = 0;
		u32 block_length = 0;
		...
		void bind_resources(mtl::command_list& cmd) override
		{
			...
			m_program->bind_uniform({ src_buffer, in_offset, block_length }, glsl::binding_set_index_compute, 0);
			m_program->bind_uniform({ dst_buffer, out_offset, block_length }, glsl::binding_set_index_compute, 1);
		}

		void set_parameters(mtl::command_list& /*cmd*/)
		{
			push_constants(0, push_constants_size, params.data);
		}

		void run(mtl::command_list& cmd, const mtl::buffer* dst, u32 out_offset, const mtl::buffer* src, u32 in_offset, u32 data_length, u32 width, u32 height, u32 depth, u32 mipmaps) override
		{
			dst_buffer = dst;
			src_buffer = src;
	...
	{
		const mtl::buffer* src = nullptr;
		const mtl::buffer* dst = nullptr;
		u32 block_length = 0;
		u32 word_count = 0;
		...
		void run(mtl::command_list& cmd, const mtl::buffer* dst, const mtl::buffer* src, u32 num_words);
	};
...
#pragma pack (pop)
		...
		void bind_resources(mtl::command_list& cmd) override
		{
			...
			m_program->bind_uniform({ src_buffer, in_offset, in_block_length }, glsl::binding_set_index_compute, 0u ^ op);
			m_program->bind_uniform({ dst_buffer, out_offset, out_block_length }, glsl::binding_set_index_compute, 1u ^ op);
		}

		void set_parameters(mtl::command_list& /*cmd*/)
		{
			push_constants(0, push_constants_size, &params);
		}

		void run(mtl::command_list& cmd, const RSX_detiler_config& config)
		{
	...
	extern std::unordered_map<u32, std::unique_ptr<mtl::compute_task>> g_compute_tasks;
	...
	// Per-frame reset hook (compute tasks hold no per-frame resources; kept for parity with VK)
	void reset_compute_tasks();

	// Destroys every cached compute task and its pipeline. Call on shutdown once the GPU is idle
	// (before mtl::get_resource_manager()->destroy() and before the device is destroyed).
